...
**Prior status:** Sprint 7 — Race Finish, Ranking & Victory Ceremony complete, including the Victory Ceremony addendum. Configurable-length races run until every participant finishes or is eliminated (gap-based warning/countdown, plus a race-wide safety-net timeout), host-authoritative final ranking (finish order, then finish time; eliminated players ranked by distance), a Podium Ceremony (top 3 only, camera movement, national flags, victory music, a dedicated champion presentation, skip button, automatic playback) synchronized identically for every player, a private per-player animated Reward Screen, and automatic return to the same lobby — all synchronized over the same `IMatchTransport` seam as Sprints 4–6. Skipping the ceremony is now individual per player and never interrupts anyone else.
