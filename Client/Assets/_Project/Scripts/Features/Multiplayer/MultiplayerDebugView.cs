...
{
    /// <summary>
    /// Debug overlay: Connected Players, Ping, Connection State, Player IDs,
    /// Match State — plus a handful of Editor/dev-build-only buttons
    /// (Create/Ready/Simulate Remote Join/Leave) so the whole Match Flow can
    /// be exercised end-to-end today with no menu UI yet in the project
    /// (same OnGUI-placeholder approach as CountdownView/RunnerDebugView).
    /// </summary>
    public sealed class MultiplayerDebugView : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            Line($"Match State: {(match != null ? match.State.ToString() : "n/a")}");
            ...
            if (lobby != null)
            {
                ...
                foreach (MatchParticipant participant in lobby.Participants)
                {
                    ...
                    Line($"  [{connectionId}] {participant.Identity.DisplayName} " +
                         $"Host:{participant.IsHost} Ready:{participant.Ready} Conn:{participant.Connection} Ping:{pingMs:F0}ms");
                }
            }

            y += 6;
            DrawControls(session, ref y, width);
        }

        private void DrawControls(SessionManager session, ref int y, int width)
        {
            if (session == null)
            {
                return;
