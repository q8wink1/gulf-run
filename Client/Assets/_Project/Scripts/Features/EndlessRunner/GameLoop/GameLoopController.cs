...
namespace GulfRun.Features.EndlessRunner.GameLoop
{
    ...
    /// session (Ready -> Running -> Paused/GameOver -> Restart). Owns the
    /// single per-frame tick ordering for every gameplay-session system, so
    /// there is never any ambiguity about MonoBehaviour Update() execution
    /// order between them — and so a future server-authoritative simulation
    /// can drive the exact same Tick sequence for multiplayer sync.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class GameLoopController : SceneSingleton<GameLoopController>
    {
        ...
        /// <summary>GameOver -> Restart -> Ready: resets every session system for a fresh attempt.</summary>
